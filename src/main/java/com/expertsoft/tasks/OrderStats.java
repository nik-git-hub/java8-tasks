...
class OrderStats {
    ...
    static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
        ...
        return orders
                .collect(Collectors.groupingBy(o -> o.getOrderItems()
                                .stream()
    ...
    static Boolean hasColorProduct(final Stream<Order> orders, final Product.Color color) {
        ...
        return orders.map(Order::getOrderItems).allMatch(orderItems ->
                orderItems.stream().map(orderItem -> orderItem.getProduct()
                        .getColor()).distinct().anyMatch(productColor -> productColor.equals(color)));
    }
    ...
    static Optional<String> mostPopularCountry(final Stream<Customer> customers) {
        ...
        return Optional.of(customers.map(Customer::getAddress).map(AddressInfo::getCountry)
                    ...
                    if (!list.isEmpty()) {
                        for (OrderItem orderItem : list) {
                            for (int i = 0; i < orderItem.getQuantity(); i++) {
                                bigDecimalList.add(orderItem.getProduct().getPrice());
                            }
                        }
                    }
